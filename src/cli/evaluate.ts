...
async function promptForOptions(): Promise<EvaluateOptions> {
  ...
  console.log(
    `  Test Set: ${testSet.name} (${testSet.summary.totalMazes} mazes)`
  );
...
async function runEvaluation(options: EvaluateOptions) {
  ...
  const evaluationList = limitArg
    ? mazesToEvaluate.slice(0, limitArg)
    : mazesToEvaluate;
...
export const evaluateCommand = new Command("evaluate")
  ...
  .option(
    "-i, --interactive",
    "Run in interactive mode (default if no options provided)"
  )
  .action(async (options) => {
    ...
    if (!dryRun && !apiKey) {
      console.error(
        chalk.red("API key required. Set OPENROUTER_API_KEY or use --api-key")
      );
      process.exit(1);
    }
    ...
    if (dryRun) {
      ...
      for (const difficulty of DIFFICULTIES) {
        const count = evaluationList.filter(
